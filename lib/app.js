...
let cmsAPI = new cms.API('http://cmsapi.app.xiaoyun.com/GpCmsApi', '100002', '8F97093B9DE32CBA569EAD6456C32A', {
    cache: false,
    fetch
})
...
const getInfoByAppId = (appId) => {
    ...
    /* eslint-disable */
    if (appId > 1000000) {
        cmsAPI._path = 'http://test-cmsapi.app.xiaoyun.com/GpCmsApi'
    }
    /* eslint-enable */
    return cmsAPI.app(appId).then(data => {
        ...
        config.HOST = 'https://weapp.apps.xiaoyun.com'
        if (appId > 1000000) {
            config.HOST = 'https://weapp-test.v2.yoo.yunpro.cn'
        }
        return config
    })
}
...
const generateAPPJSON = (config, options) => {
    ...
    return mobAPI.ui()
        .then(data => {
            console.log("data", data.body.navigation.navItemList)
