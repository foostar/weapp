...
const appJSON = require('../templates/app.json')
...
const getInfoByAppId = (appId) => {
    ...
    return cmsAPI.app(appId).then(data => {
        ...
        config.COLOR = data.themeColor.slice(1).toLowerCase()
...
const loadIcons = (list, color) => {
    return Promise.all([].concat.apply([], list.map(x => {
        ...
        return loadFile(x, path.join(__dirname, `../src/images/tabicons/icon_${parseInt(i / 2, 10)}_${i % 2 === 0 ? 'n' : 'h'}.png`))
    }))
}

const generateTabPages = (list) => {
    return new Promise((resolve, reject) => {
        ...
        return Promise.all(list.map(x => x.moduleId).map((id, i) => {
            return Promise.all([
                writeFile(path.join(__dirname, `../src/pages/tabs/tab${i}.js`), ejs.render(result.jsStr, { moduleId: id })),
                writeFile(path.join(__dirname, `../src/pages/tabs/tab${i}.wxss`), ejs.render(result.wxssStr, { moduleId: id })),
                writeFile(path.join(__dirname, `../src/pages/tabs/tab${i}.wxml`), ejs.render(result.wxmlStr, { moduleId: id }))
            ])
        }))
    })
}
            ...
            return Promise
                ...
                .then(() => {
                    if (options.useTabbar) {
                        ...
                        return generateTabPages(list)
                            .then(() => loadIcons(list, config.COLOR))
                            .then(() => {
                                return list.map((x, i) => {
                                    return {
                                        text: x.title,
